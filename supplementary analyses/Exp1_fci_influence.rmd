...
fb_data <- read.csv(file="../data/Dataset_S4_exp1_144dogs.csv")%>%
  mutate(worker=ifelse(fci_group=="1"|fci_group=="7"|fci_group=="8", "cooperative", "independent"))
view(fb_data)
...
GLMM  with FCI group as random intercept
...
p<-plot_model(full.choice.fci,  type = "re", vline.color = "black", width = 0.3, line.size = 1, ci.lvl=0.95, terms = "condition")+theme_bw()+xlab("FCI groups")+ylab("Odds ratios")

plot_model(full.choice.fci,  type = "re")

p

ggsave(p, filename="glmm_plot_144dogs_fci_re.png", width=14, height=6, scale=0.8)
